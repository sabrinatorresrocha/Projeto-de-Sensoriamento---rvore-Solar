...
// Interface para os dados históricos (para o gráfico)
...
export default function SensorPage() {
    ...
    // Estado para controlar qual arquivo está sendo apagado no momento
    const [deletingFile, setDeletingFile] = useState<string | null>(null); 

    const handleVoltar = () => {
        // Usa a função nativa do navegador para voltar, garantindo compatibilidade.
        window.history.back(); 
    };

    // Função para iniciar o download completo de todos os dados
    const handleDownloadAll = () => {
        // Redireciona para o endpoint da API com o parâmetro `download_all=true`
    ...
    const handleDeleteFile = async (filename: string) => {
        ...
        setDeletingFile(filename); // Define o arquivo que está sendo apagado

        try {
            // Requisição DELETE passando o ID do sensor e o nome do arquivo
            ...
            // Atualiza a lista de arquivos no front-end após o sucesso
    ...
    useEffect(() => {
        async function buscarDados() {
            try {
                // Requisição com o parâmetro 'history=true' para solicitar os dados históricos (limitado a 50)
                ...
                // 2. ATUALIZA DADOS HISTÓRICOS (Para o gráfico)
                const hoje = new Date();
    ...
    return (
        <main className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-8 relative"> 
            <div className="w-full max-w-6xl bg-white rounded-xl shadow-2xl p-8">
                ...
                {/* Botão de Retorno no canto superior */}
                <button
                    ...
                    {arquivos.length > 0 && (
                        <div className="mb-6 flex justify-center">
                            <button
                                ...
                                className="px-6 py-3 bg-yellow-500 text-white font-bold rounded-xl shadow-lg hover:bg-yellow-800 transition-colors text-lg w-full md:w-auto"
                            >
                    ...
                    <ul className="space-y-3">
                        {arquivos.length > 0 ? (
                            arquivos.map((arquivo, index) => (
                                <li key={index} className="flex flex-col md:flex-row items-center justify-between bg-gray-50 p-4 rounded-lg hover:bg-gray-200 transition-colors shadow-sm">
                                    <span className="font-mono text-gray-800 mb-2 md:mb-0 md:flex-1 text-center md:text-left">{arquivo}</span>
                                    
                                    <div className="flex gap-3">
                                        {/* Botão de Baixar */}
                                        <a 
                                            href={`/data/sensor${ID_SENSOR}/${arquivo}`} 
                                            download={arquivo} 
                                            className="flex items-center px-3 py-2 bg-yellow-500 text-white font-semibold rounded-md hover:bg-yellow-800 transition-colors text-sm"
                                        >
                                            <Download size={16} className="mr-1" /> Baixar
                                        </a>
                                        
                                        {/* Botão de Apagar Arquivo Individual */}
                                        <button
